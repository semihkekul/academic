		...
		for(int y = 0; y< MATRIX_DIM; y++)
			...
			for(int i =offsetA; rowA[i] == idx;i++ )
				...
				for(int a = 0 ; a<DATA_SIZE ;a++ )
					...
					Z[MATRIX_DIM*idx+y] = val; //bu satýrý kaldýrýnca sorunsuz calisio
				}
			}
...
int main(void)
	...
	file = fopen("C:\\gate713\\coo5000\\coo5000val.txt","r");
	...
	fclose(file);
		...
		file = fopen("C:\\gate713\\coo5000\\coo5000row.txt","r");
	if (file==NULL) perror("file");
	for( iter = 0; iter < DATA_SIZE; iter++)
		{
	...
	fclose(file);
		...
		file = fopen("C:\\gate713\\coo5000\\coo5000col.txt","r");
	if (file==NULL) perror("file");
	for( iter = 0; iter < DATA_SIZE; iter++)
		{
